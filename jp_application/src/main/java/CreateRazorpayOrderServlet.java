...
public class CreateRazorpayOrderServlet extends HttpServlet {
	...
	private static final String RAZORPAY_KEY_ID = "rzp_test_S5iOpvQklhHykr";
	private static final String RAZORPAY_KEY_SECRET = "5BtL71yriYw7tb9QtQnlsTbk";
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			String orderJson = String.format(
					...
					amount, currency != null ? currency : "INR", receiptId,
					customerName != null ? customerName : "Customer",
					customerEmail != null ? customerEmail : "customer@example.com", userId != null ? userId : "0");
			...
			if (responseCode >= 200 && responseCode < 300) {
				...
				out.println(String.format(
						...
						orderId, responseAmount != null ? responseAmount : amount,
						responseCurrency != null ? responseCurrency : currency, RAZORPAY_KEY_ID));
			} else {
				out.println(String.format("{\"success\":false,\"message\":\"Razorpay API Error (Code: %d): %s\"}",
						responseCode, apiResponse.length() > 200 ? apiResponse.substring(0, 200) : apiResponse));
			}
