...
<body>
	...
	<div class="container mt-4">
		...
		<div class="d-flex justify-content-between align-items-center mb-4">
			...
			<div>
				<a href="products.jsp" class="btn btn-primary"> <i
					class="bi bi-plus-circle"></i> Shop More
				</a>
		...
		try {
			...
			statsConn = DriverManager.getConnection("jdbc:mysql://localhost:3306/ecom", "root", "");
			...
			statsRs = statsPs.executeQuery();
			if (statsRs.next())
				pendingOrders = statsRs.getInt("count");
			...
			statsRs = statsPs.executeQuery();
			if (statsRs.next())
				deliveredOrders = statsRs.getInt("count");
			...
			statsRs = statsPs.executeQuery();
			if (statsRs.next())
				cancelledOrders = statsRs.getInt("count");
			...
			statsRs = statsPs.executeQuery();
			if (statsRs.next())
				totalSpent = statsRs.getDouble("total");
		...
		<div class="alert alert-info">
			<div class="d-flex justify-content-between align-items-center">
				<div>
					<h5 class="mb-1">
						<i class="bi bi-currency-rupee"></i> Total Amount Spent
					</h5>
					<p class="mb-0">
						You've spent a total of <strong>₹<%=String.format("%.2f", totalSpent)%></strong>
						on
						<%=totalOrders%>
						orders
					</p>
				...
				<div class="col-md-3">
					<label for="status" class="form-label">Status</label> <select
						...
						<option value="pending"
							<%="pending".equals(filterStatus) ? "selected" : ""%>>Pending</option>
						<option value="confirmed"
							<%="confirmed".equals(filterStatus) ? "selected" : ""%>>Confirmed</option>
						<option value="processing"
							<%="processing".equals(filterStatus) ? "selected" : ""%>>Processing</option>
						<option value="shipped"
							<%="shipped".equals(filterStatus) ? "selected" : ""%>>Shipped</option>
						<option value="delivered"
							<%="delivered".equals(filterStatus) ? "selected" : ""%>>Delivered</option>
						<option value="cancelled"
							<%="cancelled".equals(filterStatus) ? "selected" : ""%>>Cancelled</option>
					</select>
				</div>
				<div class="col-md-3">
					<label for="date" class="form-label">Date Range</label> <select
						class="form-select" id="date" name="date">
						<option value="">All Time</option>
						<option value="today"
							<%="today".equals(filterDate) ? "selected" : ""%>>Today</option>
						<option value="week"
							<%="week".equals(filterDate) ? "selected" : ""%>>This
							Week</option>
						<option value="month"
							<%="month".equals(filterDate) ? "selected" : ""%>>This
							Month</option>
						<option value="year"
							<%="year".equals(filterDate) ? "selected" : ""%>>This
							Year</option>
					</select>
				</div>
				<div class="col-md-4">
					<label for="search" class="form-label">Search Orders</label> <input
		...
		<div class="card">
			<div class="card-header bg-light">
				<div class="d-flex justify-content-between align-items-center">
					<h5 class="mb-0">
						<i class="bi bi-list-ul"></i> Your Orders
					</h5>
			...
			<div class="card-body">
				...
				try {
					Class.forName("com.mysql.cj.jdbc.Driver");
					conn = DriverManager.getConnection("jdbc:mysql://localhost:3306/ecom", "root", "");
					...
					case "month" :
						sqlBuilder
								.append("AND MONTH(o.order_date) = MONTH(CURDATE()) AND YEAR(o.order_date) = YEAR(CURDATE()) ");
						break;
					case "year" :
					...
					if (searchQuery != null && !searchQuery.isEmpty()) {
						sqlBuilder.append(
						"AND (o.id LIKE ? OR EXISTS (SELECT 1 FROM order_items oi JOIN products p ON oi.product_id = p.id WHERE oi.order_id = o.id AND p.name LIKE ?)) ");
					...
					<div class="card-body">
						<div class="row">
							...
							<div class="col-md-8">
								...
								%>
								<p class="mb-2">
									<small class="text-muted"> <i class="bi bi-cart"></i> <%=productNames.length() > 100 ? productNames.substring(0, 100) + "..." : productNames%>
									</small>
								</p>
								<%
								}
								...
								<h4 class="text-dark mb-0">
									₹<%=String.format("%.2f", totalAmount)%></h4>
							</div>

							<!-- Action Buttons -->
							<div class="col-md-4">
								<div class="order-actions justify-content-end">
									<a href="order-confirmation.jsp?order_id=<%=orderId%>"
										class="btn btn-outline-primary"> <i class="bi bi-eye"></i>
										View
									</a>
								...
								<div class="mt-3 text-end">
									...
									%>
									<small class="text-info"> <i class="bi bi-info-circle"></i>
									...
									<small class="text-success"> <i
										class="bi bi-check-circle"></i> Delivered successfully
									</small>
									<%
				...
				<div class="empty-state">
					...
					<div class="d-grid gap-2 col-md-6 mx-auto">
						<a href="products.jsp" class="btn btn-primary btn-lg"> <i
							class="bi bi-shop"></i> Start Shopping
						</a>
						<%
						if (filterStatus != null || filterDate != null || searchQuery != null) {
						%>
				...
				<div class="alert alert-danger">
					<div class="d-flex align-items-center">
						...
						<div>
							...
							<p class="mb-0">An error occurred while loading your order
								history. Please try again later.</p>
						</div>
					</div>
					<hr>
					<div class="d-flex gap-2">
						<a href="index.jsp" class="btn btn-primary"> <i
							class="bi bi-house"></i> Return Home
						</a>
		...
		<footer class="bg-light mt-5 py-4 border-top">
			<div class="container">
				<div class="row">
					<div class="col-md-6">
						<h5>Order Support</h5>
						<p class="text-muted small mb-2">Need help with your orders?
							Contact our support team.</p>
						<p class="mb-0">
							<i class="bi bi-envelope"></i> support@ecommerce.com<br> <i
								class="bi bi-telephone"></i> +91 9876543210
						</p>
					</div>
					<div class="col-md-6 text-end">
						<p class="text-muted small mb-0">&copy; 2024 Ecommerce. All
							rights reserved.</p>
						<p class="small mb-0">
							<a href="privacy.jsp" class="text-decoration-none me-3">Privacy
								Policy</a> <a href="terms.jsp" class="text-decoration-none">Terms
								of Service</a>
						</p>
					</div>
				</div>
